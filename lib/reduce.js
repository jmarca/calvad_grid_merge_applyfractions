...
function apply_fractions(task,cb){
    ...
    _.each(task.aadt_store,function(aadt_record,index){
        ...
        _.each(nhh_vars,function(k2){
            // multiply by nhh
            _.each(task.fractions,function(record,timestamp){
                task.accum[timestamp][road_class][k2] = Math.round(record.nhh * scale.nhh * aadt_record[k2])
                return null
            })
            return null
        })
        _.each(n_vars,function(k2){
            // multiply by n
            _.each(task.fractions,function(record,timestamp){
                task.accum[timestamp][road_class][k2] = Math.round(record.n * scale.n * aadt_record[k2])
                return null
            })
            return null
        })
        _.each(hh_vars,function(k2){
            // multiply by hh
            _.each(task.fractions,function(record,timestamp){
                task.accum[timestamp][road_class][k2] = Math.round(record.hh * scale.hh * aadt_record[k2])
                return null
            })
            return null
        })
    ...
    _.forEach(task.fractions, function(cell_fractions, cellid){
        ...
        _.forEach(aadt,function(aadt_record,index){
            ...
            task.accum[cellid][road_class].sum_lane_miles
                = Math.round(aadt_record.sum_lane_miles)


            _.each(nhh_vars,function(k2){
                // multiply by nhh
                task.accum[cellid][road_class][k2] =
                    Math.round(cell_fractions.nhh * grid_scale.nhh * aadt_record[k2])
                return null
            })
            _.each(n_vars,function(k2){
                // multiply by n
                task.accum[cellid][road_class][k2] =
                    Math.round(cell_fractions.n * grid_scale.n * aadt_record[k2])
                return null
            })
            ...
            _.each(hh_vars,function(k2){
                // multiply by hh
                task.accum[cellid][road_class][k2] =
                    Math.round(cell_fractions.hh * grid_scale.hh * aadt_record[k2])
                return null
            })
            return null
        })
...
function reduce(memo,item,cb){
    ...
    _.each(item.accum,function(roads,ts){
        ...
        if(memo[ts]===undefined){
            memo[ts]={}
            _.forEach(roads,function(v,road_class){
                memo[ts][road_class]={}
                _.forEach(v,function(vv,kk){
                    memo[ts][road_class][kk] = Math.round(vv)
                    return null
                })
                return null
            })
            ...
            _.each(roads,function(record,road_class){
                if(memo[ts][road_class]===undefined){
                    memo[ts][road_class]={}
                    _.forEach(record,function(vv,kk){
                        memo[ts][road_class][kk] = Math.round(vv)
                        return null
                    })
                }else{
                    _.each(record,function(v,k){
                        memo[ts][road_class][k] += Math.round(v)
                    })
                }
        ...
        record.forEach(function(r){
            if(memo[ts].detector_based===undefined){
                memo[ts].detector_based={//'n':r[unmapper.n]
                    'n_mt':Math.round(r[unmapper.n])
                    ,'hh_mt':Math.round(r[unmapper.hh])
                    ,'nhh_mt':Math.round(r[unmapper.not_hh])
                    ,'lane_miles':r[unmapper.lane_miles]
                    ,'miles':r[unmapper.miles]
                }
            }else{
                //memo[ts].detector_based.n      += r[unmapper.n]
                memo[ts].detector_based.n_mt   += Math.round(r[unmapper.n])
                memo[ts].detector_based.hh_mt  += Math.round(r[unmapper.hh])
                memo[ts].detector_based.nhh_mt += Math.round(r[unmapper.not_hh])
    ...
    _.each(item.detector_data,function(record,cellid){
        ...
        record.forEach(function(r){
            ...
            if(memo[cellid].detector_based===undefined){
                ...
                //memo[cellid].detector_based.n      += r[unmapper.n]
                memo[cellid].detector_based.n_mt   += Math.round(r[unmapper.n])
                memo[cellid].detector_based.hh_mt  += Math.round(r[unmapper.hh])
                memo[cellid].detector_based.nhh_mt += Math.round(r[unmapper.not_hh])
