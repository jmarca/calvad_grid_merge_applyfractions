 ...
 * hpms data.  the previous "totals" data from hpms info is renamed
 * here to hpms_totals
 *
 * There is not grand total, but to get that, sum together the
 * hpms_totals and the detector_based rows
 *
 * this function is synchronous, but you can call it like async if you
...
function reduce(memo,item,cb){
    ...
    // rename totals to hpms_totals
    _.each(memo,function(v,ts){
        v.hpms_totals = v.totals
        delete v.totals
        return null
    })
