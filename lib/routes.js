...
function grid_hpms_handler_one_hour(config,hpmsfiles,app,cb){
    ...
    q.awaitAll(function(e,data){
        ...
        app.get('/hpms/data_by_hr/:yr/:month/:day/:hour.:format?'
                ,function(req,res,next){
                    var task,f
                    // closure around request object
                    function generated_response_handler(e,t){
                        var memo = {}
                        if(e) return next(e)
                    ...
                    task=task_init_one_hour(req)
                    task.options = config
                    // first, check if csv requested, perhaps json already exists?
                    if(csv_test.test(req.params.format)){
                        // respond with csv
                        res.writeHead(200, { 'Content-Type': 'text/csv' })
                        f = 'public/hpms/data_by_hr/'
                            +req.params.yr +'/'
                            +req.params.month +'/'
                            +req.params.day +'/'
                            +req.params.hour +'.json'
                        // debugging
                        console.log(f)
                        fs.stat(f,function(estat,stat){
                            if(estat){
                                throw new Error(estat)
                            }
                            if(stat.isFile()){
                                fs.readFile(f,function(eread,res){
                                    if(e){
                                        throw new Error(eread)
                                    }
                                    var memo = JSON.parse(res)
                                    res.writeHead(200, { 'Content-Type': 'text/csv' })
                                    stringify(arrayifier(memo),function(e,arr){
                                        res.end(arr)
                                    })
                                ...
                                // if json file exists, and writing csv, can bail on the rest, all done here
                                return null
                            }
                            // else, create the file, and let the handler dump as csv or json
                            fractions_handler_one_hour(
                                hpmsgrids[task.year]
                                ,task
                                ,generated_response_handler);
                            return null
                        })
                    }else{ // not a csv request, so need to generate the JSON
                        fractions_handler_one_hour(
                            hpmsgrids[task.year]
                            ,task
                            ,generated_response_handler)
