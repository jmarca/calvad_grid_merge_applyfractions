...
function fractions_handler_one_hour(hpmsgrids){
    // return function(task,cb){
    ...
    //                     // this next thing is actually synchronous, okay
    //                     // to call like this without wrapping in queue()
    //                     reduce.apply_fractions_one_hour(task,hpmsgrids,function(eee){
    //                         if(eee) return cb(eee)
    //                         return cb(null,task)
    //                     })
    //                     return null
...
function grid_hpms_handler_one_hour(config,hpmsfiles,app,cb){
    ...
    //                 task.options = config
    //                 queue()
    //                     .defer(fractions_handler_one_hour(hpmsgrids[task.year])
    //                            ,task)
    //                     .await(function(e,t){
    //                         var memo = {}
    //                         if(e) return next(e)
    //                         // iterate over each grid cell
    //                         var memo = _.reduce()reduce.reduce({},task)
    //                     var csv_test = /^csv$/i;
    //                     if(csv_test.test(req.params.format)){
    //                         // respond with csv
    //                         res.writeHead(200, { 'Content-Type': 'text/csv' })
